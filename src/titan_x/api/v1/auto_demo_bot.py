...
async def run_demo_cycle(
    ...
    try:
        ...
        await session.rollback()
        raise HTTPException(status_code=500, detail=f"Auto demo bot cycle failed: {type(exc).__name__}") from exc
...
async def demo_bot_status(
    ...
    return {"enabled": True, "mode": "paper_demo", "max_cycles": 15, "paper_account": account is not None}
