...
async def _live_prices(session, items) -> dict[str, float]:
...
async def list_recommendations(symbol: str | None = None, direction: str | None = None, status: str | None = None, recommendation_type: str | None = None, timeframe: str | None = None, min_confidence: float | None = None, min_score: float | None = None, source: str | None = None, risk_level: str | None = None, decision: str | None = None, outcome: str | None = None, sort_by: str = "generated_at", sort_desc: bool = True, limit: int = Query(default=50, le=200), offset: int = Query(default=0, ge=0), session=Depends(deps.get_session), _: User = Depends(deps.get_current_active_user)):
...
async def get_top_recommendations(limit: int = Query(default=10, le=50), status: str = "active", min_score: float | None = None, session=Depends(deps.get_session), _: User = Depends(deps.get_current_active_user)):
    svc = RecommendationService(session)
    items = await svc.get_top_recommendations(limit=limit, status=status, min_score=min_score)
    live = await _live_prices(session, items)
...
async def trigger_scan(background_tasks: BackgroundTasks, max_age_minutes: int | None = Query(default=60, ge=0), limit: int | None = Query(default=None, ge=1, le=2000), sync: bool = Query(default=False), session_factory=Depends(get_app_session_factory), _: User = Depends(deps.get_current_active_user)):
    ...
    if sync:
        ...
        last = state.get("last") or {"started": True, "universe": 0, "scanned": 0, "stored": 0, "insufficient_data": 0, "no_trade": 0, "failed": 0, "skipped_fresh": 0, "used_fallback_universe": False, "finished_at": datetime.now(timezone.utc).isoformat()}
        return {"last": last, "last_error": state.get("last_error"), "running": state.get("running", False)}
...
async def get_recommendation_history(symbol: str = Query(...), limit: int = Query(default=50, le=200), offset: int = Query(default=0, ge=0), session=Depends(deps.get_session), _: User = Depends(deps.get_current_active_user)):
    svc = RecommendationService(session)
    items = await svc.get_recommendation_history(symbol=symbol, limit=limit, offset=offset)
...
async def get_recommendations_by_symbol(symbol: str, status: str | None = "active", limit: int = Query(default=20, le=100), offset: int = Query(default=0, ge=0), session=Depends(deps.get_session), _: User = Depends(deps.get_current_active_user)):
    svc = RecommendationService(session)
    items = await svc.get_recommendations_by_symbol(symbol=symbol, status=status, limit=limit, offset=offset)
...
async def analyze_symbol(symbol: str, session=Depends(deps.get_session), _: User = Depends(deps.get_current_active_user)):
    """Run the six-pillar analysis using real NSE historical data only."""
    symbol = symbol.upper()
    engine = AIRecommendationEngine()
    provider = JugaadNSEProvider()
    try:
        points = await provider.get_historical_prices(symbol, interval="1d", start=None, end=None, synthetic_ok=False)
    except Exception as exc:
        raise HTTPException(503, f"Live NSE price data unavailable for '{symbol}': {exc}") from exc
    finally:
        await provider.close()
    if not points:
        raise HTTPException(404, f"No live NSE price data available for symbol '{symbol}'")
    ...
    if breadth:
        ...
        breadth_ctx = {"index_strength_score": breadth.index_strength_score or 50.0, "adv_decl_ratio": adv}
    rec = engine.build(symbol, bars, fundamentals=fundamentals, sector_ctx=sector_ctx, breadth_ctx=breadth_ctx)
    rec["data_points"] = len(points)
    live_quote = await MarketDataService(session).get_quote(symbol)
    live_price = float(live_quote["last_price"])
    rec["entry_price"] = live_price
    return {"symbol": symbol, "recommendation": {"signal": rec["signal"], "direction": rec["direction"], "score": rec["score"], "confidence": rec["confidence"], "calibrated_probability": rec["calibrated_probability"], "conviction": rec["conviction"], "entry_price": live_price, "price_target": rec["price_target"], "stop_price": rec["stop_price"], "risk_reward": rec["risk_reward"], "holding_period_days": rec["holding_period_days"], "expected_return_pct": rec["expected_return_pct"], "risk_level": rec["risk_level"], "no_trade": rec["no_trade"], "rejection_reasons": rec["rejection_reasons"], "evidence": rec["evidence"], "caution": rec["caution"], "as_of_date": rec["as_of_date"], "data_points": rec["data_points"]}, "explainability": rec["explainability"]}
...
async def set_decision(rec_id: int, decision: str = Query(...), decision_reason: str | None = None, session=Depends(deps.get_session), _: User = Depends(deps.get_current_active_user)):
    rec = await RecommendationService(session).set_decision(rec_id=rec_id, decision=decision, decision_reason=decision_reason)
    if not rec:
...
async def set_outcome(rec_id: int, outcome: str = Query(...), actual_outcome_pnl: float | None = None, outcome_details: str | None = None, session=Depends(deps.get_session), _: User = Depends(deps.get_current_active_user)):
    rec = await RecommendationService(session).set_outcome(rec_id=rec_id, outcome=outcome, actual_outcome_pnl=actual_outcome_pnl, outcome_details=outcome_details)
    if not rec:
        raise HTTPException(404, "Recommendation not found")
    return _rec_dict(rec)


def _rec_dict(r, live_price: float | None = None) -> dict:
