...
# The technical engine's weighted factors top out below 95, so a 95 gate
# made both recommendation modes mathematically unreachable.
TECHNICAL_THRESHOLD = 85
    ...
    rows = (
        await session.execute(
            ...
            .where(
                ...
                Recommendation.recommendation_type == "intraday",
                Recommendation.source == "yahoo",
            )
            .order_by(Recommendation.generated_at.desc(), Recommendation.score.desc())
            .limit(limit)
        )
    ).scalars().all()
    result = []
...
async def _intraday_segment(symbols, segment_id):
    ...
    # Yahoo's end bound is commonly exclusive; tomorrow ensures today's bars
    # are included when the market has traded today.
        ...
        async with sem:
            try:
                ...
                technical = await asyncio.to_thread(
                    score_technical_strength, bars_from_records(points), mode="intraday"
                )
    ...
    async with session_factory() as session:
        ...
        for item in results:
            metadata = {
                ...
                "technical_pillar_score": item.get("technical_pillar_score", item.get("score")),
            }
...
async def _run_cached_scan(symbols, session_factory):
    ...
    async with _cache_lock:
        _cache_state.update(
            ...
            universe=len(symbols), scanned=0, successful=0, failed=0,
            segment_progress={str(i): "queued" for i in range(1, SCAN_SEGMENTS + 1)}, last_error=None,
        )
        try:
            ...
            return {
                "started": True, "universe": len(symbols), "scanned": len(symbols),
                "successful": len(results), "failed": _cache_state["failed"],
                "cache_count": len(_intraday_cache), "provider": "yahoo",
                "technical_threshold": TECHNICAL_THRESHOLD, "persisted": bool(results),
            }
        except Exception as exc:
    ...
    async with _delivery_lock:
        ...
        async def runner():
            try:
                ...
                import structlog
                structlog.get_logger("recommendation.strict").error(
                    "background_delivery_scan_failed", error=str(exc)
                )
...
async def _strict_delivery_cache(session, limit):
    ...
    for r in rows:
        ...
        technical = float(gate.get("delivery_score") or 0)
        ...
        result.append({
            ...
            "current_price": r.current_price, "price_target": r.price_target,
            "risk_level": r.risk_level, "source": "yahoo",
            "generated_at": r.generated_at.isoformat() if r.generated_at else None,
        })
        if len(result) >= limit:
            break
...
def _scan_status_payload(status):
    ...
    return {
        ...
        "progress_pct": round(100 * last.get("scanned", 0) / universe, 1) if universe else 0,
...
async def intraday_recommendations(
    ...
    session=Depends(deps.get_session), session_factory=Depends(get_app_session_factory),
    ...
    return {
        "recommendations": _intraday_cache[:limit], "count": min(limit, len(_intraday_cache)),
        "universe_scanned": len(symbols), "scan_segments": SCAN_SEGMENTS,
        "cache_ready": bool(_intraday_cache), "scan_running": _cache_state["running"],
        "persistent_cache": True, "provider": "yahoo", "live": True,
        "strict_technical_threshold": TECHNICAL_THRESHOLD, "strict_gate": "technical_pillar>=85",
    }


@router.post("/recommendations/intraday/refresh")
async def refresh_intraday_recommendations(
    segment: str = Query("equity", pattern=r"^(equity|fno)$"),
    session=Depends(deps.get_session), session_factory=Depends(get_app_session_factory),
    _: User = Depends(deps.get_current_active_user),
):
    ...
    asyncio.create_task(_run_cached_scan(symbols, session_factory))
    return {
        "started": True, "universe": len(symbols), "scan_segments": SCAN_SEGMENTS,
        "technical_threshold": TECHNICAL_THRESHOLD, "provider": "yahoo", "persistent_cache": True,
        "message": "Full-market intraday scan started in background; Technical Pillar >=85 results are persisted",
    }
...
async def intraday_scan_status(session=Depends(deps.get_session), _: User = Depends(deps.get_current_active_user)):
    if not _intraday_cache:
        _intraday_cache.extend(await _load_persisted_cache(session, 100))
    return {"provider": "yahoo", "scan_segments": SCAN_SEGMENTS, "cache_count": len(_intraday_cache),
            "persistent_cache": True, "technical_threshold": TECHNICAL_THRESHOLD, **_cache_state}
...
async def strict_recommendations(
    ...
    if mode == "intraday":
        ...
        return {
            "recommendations": _intraday_cache[:limit], "count": min(limit, len(_intraday_cache)),
            "mode": mode, "segment": segment, "provider": "yahoo", "scanning": _cache_state["running"],
            "scan_running": _cache_state["running"], "cache_ready": bool(_intraday_cache),
            "universe_scanned": len(symbols), "scan_segments": SCAN_SEGMENTS,
            "scan_status": {
                "scanned": _cache_state["scanned"], "universe_size": _cache_state["universe"],
                "progress_pct": round(100 * _cache_state["scanned"] / _cache_state["universe"], 1) if _cache_state["universe"] else 0,
                "error": _cache_state["last_error"],
            },
            "persistent_cache": True, "live": True,
            "strict_technical_threshold": TECHNICAL_THRESHOLD, "strict_gate": "technical_pillar>=85",
        }
    ...
    if not recommendations and not delivery_running:
        recently_attempted = (
            _delivery_last_attempt is not None
            and (datetime.now(timezone.utc) - _delivery_last_attempt).total_seconds() < 60
        )
    ...
    return {
        "recommendations": recommendations, "count": len(recommendations), "mode": mode,
        "segment": segment, "provider": "yahoo", "scanning": bool(status.get("running") or delivery_running),
        "scan_status": _scan_status_payload(status),
        "strict_technical_threshold": TECHNICAL_THRESHOLD,
        "strict_gate": "delivery_technical_pillar>=85",
    }
