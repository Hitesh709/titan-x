...
async def _symbols(session,limit):
    rows=(await session.execute(select(Company.symbol).where(Company.status=="active").where(Company.exchange.in_(["NSE","BSE"])).order_by(Company.symbol).limit(limit))).all(); return [str(r[0]).upper() for r in rows if r[0]]
...
async def intraday_recommendations(segment:str=Query("equity",pattern=r"^(equity|fno)$"),limit:int=Query(20,ge=1,le=100),session=Depends(deps.get_session),_:User=Depends(deps.get_current_active_user)):
    ...
    r=await _intraday(symbols,limit); return {"recommendations":r,"count":len(r),"segment":segment,"provider":"yahoo","live":True}
...
async def strict_recommendations(mode:str=Query("delivery",pattern=r"^(delivery|intraday)$"),segment:str=Query("equity",pattern=r"^(equity|fno)$"),limit:int=Query(20,ge=1,le=100),session_factory=Depends(get_app_session_factory),_:User=Depends(deps.get_current_active_user)):
    if mode=="intraday":
        async with session_factory() as s:symbols=await _symbols(s,limit)
        r=await _intraday(symbols,limit); return {"recommendations":r,"count":len(r),"mode":mode,"segment":segment,"provider":"yahoo","live":True}
