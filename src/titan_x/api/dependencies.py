...
async def require_api_key(supplied_key: Annotated[str | None, Security(api_key_header)], credentials: Annotated[HTTPAuthorizationCredentials | None, Security(bearer_scheme)], settings: Annotated[Settings, Depends(get_settings)]) -> None:
    if supplied_key is not None and secrets.compare_digest(supplied_key, settings.api_key.get_secret_value()): return
    if credentials is not None:
        try: payload = decode_token(credentials.credentials, settings.jwt_secret_key.get_secret_value(), settings.jwt_algorithm)
        except ValueError: payload = None
        if payload and payload.get("type") == "access": return
    raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid API key")

async def request_session(request: Request) -> AsyncIterator[AsyncSession]:
    session_factory: async_sessionmaker[AsyncSession] = request.app.state.session_factory
    async for session in get_session(session_factory): yield session
...
async def get_current_user(credentials: Annotated[HTTPAuthorizationCredentials | None, Security(bearer_scheme)], session: Annotated[AsyncSession, Depends(request_session)], settings: Annotated[Settings, Depends(get_settings)]) -> User:
    ...
    try: payload = decode_token(credentials.credentials, settings.jwt_secret_key.get_secret_value(), settings.jwt_algorithm)
    except ValueError as exc: raise HTTPException(status_code=401, detail="Invalid or expired token", headers={"WWW-Authenticate":"Bearer"}) from exc
    if payload.get("type") != "access": raise HTTPException(status_code=401, detail="Invalid token type", headers={"WWW-Authenticate":"Bearer"})
    user = await BaseRepository(session, User).get(int(payload["sub"]))
    if user is None: raise HTTPException(status_code=404, detail="User not found")
    return user

async def get_current_active_user(current_user: Annotated[User, Depends(get_current_user)]) -> User:
    if not current_user.is_active: raise HTTPException(status_code=403, detail="Inactive user")
    return current_user

def get_redis(request: Request) -> Redis: return request.app.state.redis
async def get_user_repository(session: Annotated[AsyncSession, Depends(request_session)]) -> BaseRepository[User]: return BaseRepository(session, User)
def get_health_service(session: Annotated[AsyncSession, Depends(request_session)], redis: Annotated[Redis, Depends(get_redis)]) -> HealthService: return HealthService(SqlAlchemyRedisHealthRepository(session, redis))
def get_auth_service(session: Annotated[AsyncSession, Depends(request_session)], settings: Annotated[Settings, Depends(get_settings)]) -> AuthService: return AuthService(session, settings)
def get_user_service(session: Annotated[AsyncSession, Depends(request_session)], settings: Annotated[Settings, Depends(get_settings)]) -> UserService: return UserService(session, settings)
def get_qr_auth_service(session: Annotated[AsyncSession, Depends(request_session)], settings: Annotated[Settings, Depends(get_settings)]) -> QRAuthService: return QRAuthService(session, settings)
def get_rate_limiter(request: Request) -> RateLimiter | None:
    redis = getattr(request.app.state, "redis", None); return RateLimiter(redis) if redis is not None else None
def get_brute_force_protector(request: Request) -> BruteForceProtector | None:
    redis = getattr(request.app.state, "redis", None); return BruteForceProtector(redis) if redis is not None else None
def get_cache(request: Request) -> RedisCache: return request.app.state.cache
def get_session_store(request: Request) -> RedisSessionStore: return request.app.state.session_store
def get_task_queue(request: Request) -> TaskQueue | None: return getattr(request.app.state, "task_queue", None)
def get_notification_delivery_service(settings: Annotated[Settings, Depends(get_settings)]) -> NotificationDeliveryService: return NotificationDeliveryService(settings)
def get_alert_evaluation_service(session: Annotated[AsyncSession, Depends(request_session)], delivery: Annotated[NotificationDeliveryService, Depends(get_notification_delivery_service)]) -> AlertEvaluationService: return AlertEvaluationService(session, delivery)
def get_notification_service(session: Annotated[AsyncSession, Depends(request_session)], settings: Annotated[Settings, Depends(get_settings)]) -> NotificationService: return NotificationService(session, settings)

_SESSION_SERVICE_REGISTRY: dict[str, type] = {
    "ai_registry_service": AIModelRegistryService, "audit_service": AuditService, "scheduler_service": SchedulerService, "company_service": CompanyService,
    "price_service": PriceService, "corporate_action_service": CorporateActionService, "news_engine": NewsEngine, "portfolio_engine": PortfolioEngine,
    "report_generator": ReportGenerator, "prediction_engine": PredictionEngine, "explainability_engine": ExplainabilityEngine,
    "explainability_dashboard_service": ExplainabilityDashboardService, "ensemble_ai_engine": EnsembleAIEngine, "decision_engine": DecisionEngine,
    "risk_engine": RiskEngine, "historical_similarity_engine": HistoricalSimilarityEngine, "pattern_recognition_engine": PatternRecognitionEngine,
    "market_breadth_engine": MarketBreadthEngine, "sector_engine": SectorEngine, "fundamental_engine": FundamentalEngine,
    "technical_indicator_engine": TechnicalIndicatorEngine, "news_nlp_engine": NewsNLPEngine, "financial_statement_engine": FinancialStatementEngine,
    "data_io_service": DataImportExportService, "financial_analysis_service": FinancialAnalysisService, "corporate_action_detector": CorporateActionDetector,
    "corporate_action_engine": CorporateActionEngine, "intraday_service": IntradayService, "preference_service": PreferenceService,
    "broker_integration_service": BrokerIntegrationService, "backtest_engine": BacktestEngine, "strategy_builder": StrategyBuilder,
    "optimization_engine": OptimizationEngine, "market_data_service": MarketDataService, "learning_engine": LearningEngine,
    "knowledge_graph_service": KnowledgeGraphService, "corporate_tracking_service": CorporateTrackingService, "order_service": OrderService,
...
def _make_session_factory(name: str, service_cls: type) -> Callable[..., object]:
    async def _factory(session: Annotated[AsyncSession, Depends(request_session)]) -> object: return service_cls(session)
    _factory.__name__ = f"get_{name}"; _factory.__qualname__ = _factory.__name__; _factory.__doc__ = f"Provide a {service_cls.__name__} bound to the request session."; return _factory
for _service_name, _service_cls in _SESSION_SERVICE_REGISTRY.items(): globals()[f"get_{_service_name}"] = _make_session_factory(_service_name, _service_cls)
