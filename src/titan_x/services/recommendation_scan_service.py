...
# One coordinated scan with bounded symbol concurrency. The premium 99 tier
# needs access to the 75-80 Technical Pillar band, so persist live candidates
# from 75 upward and apply the exact customer entitlement at read time.
FAST_GATE_SCORE = 75.0
...
class RecommendationScanService:
    ...
    async def _scan(self, max_age_minutes, limit):
        ...
        counters = {"scanned": 0, "live_data_symbols": 0, "fast_passed": 0, "stored": 0, "insufficient_data": 0, "no_trade": 0, "failed": 0}
        errors = []
        ...
        try:
            for start in range(0, len(stale), BATCH_SIZE):
                ...
                recs = await self._process_batch(batch, provider, sector, breadth, counters, errors, counter_lock)
        ...
        return {
            "started": True, "universe": len(instruments), "scanned": counters["scanned"],
            "live_data_symbols": counters["live_data_symbols"], "fast_gate": FAST_GATE_SCORE,
            "fast_passed": counters["fast_passed"], "deep_scanned": counters["fast_passed"],
            "stored": counters["stored"], "insufficient_data": counters["insufficient_data"],
            "no_trade": counters["no_trade"], "failed": counters["failed"], "batch_failed": 0,
            "skipped_fresh": len(instruments) - len(stale), "first_error": errors[0] if errors else None,
            "scan_segments": 1, "batch_size": BATCH_SIZE, "symbol_concurrency": SYMBOL_CONCURRENCY,
            "data_quality": {"synthetic_data_used": False, "live_only_fast_gate": True, "provider": "Yahoo Finance", "delivery_interval": "1d (24h)"},
        }
    ...
    async def _process_batch(self, batch, provider, sector, breadth, counters, errors, counter_lock):
        ...
        for instrument, points, error in fetched:
            ...
            try:
                ...
                rec.update({"exchange": exchange, "yahoo_symbol": yahoo_symbol, "data_points": len(points), "fast_technical_gate": {"threshold": FAST_GATE_SCORE, "technical_pillar_score": selected, "delivery_score": selected, "selected_score": selected, "direction": delivery.direction, "label": delivery.label, "interval": "1d", "window": "24h"}})
                ...
                if rec.get("no_trade"):
                    ...
                    if not rec.get("risk_level"): rec["risk_level"] = "Medium"
                else:
    ...
    async def _store(self, rec, svc):
        ...
        metadata = {
            "signal": signal, "exchange": "NSE", "yahoo_symbol": rec.get("yahoo_symbol"), "as_of_date": rec.get("as_of_date"),
            "data_points": rec.get("data_points", 0), "evidence": rec.get("evidence"), "caution": rec.get("caution"),
            "returns": rec.get("returns"), "indicators": rec.get("indicators"), "factors": rec.get("factors"),
            "pillar_scores": rec.get("pillar_scores") or rec.get("pillars"), "fast_technical_gate": rec.get("fast_technical_gate"), "timeframe": "24h",
        }
