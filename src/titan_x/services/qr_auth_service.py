...
class QRAuthService:
    ...
    def __init__(self, session: AsyncSession, settings: Settings) -> None:
        ...
        self._device_repo = BaseRepository(session, UserDevice)
    ...
    async def create_challenge(self, browser_session: str, ip_address: str | None, user_agent: str | None) -> tuple[AuthChallenge, str, str]:
        ...
        challenge = await self._challenge_repo.create(
            ...
            challenge_hash=self._hash(raw_challenge),
    ...
    async def register_device(self, user: User, device_name: str, public_key: str) -> UserDevice:
        ...
        if not isinstance(key, Ed25519PublicKey):
            raise ValueError("Only Ed25519 device public keys are supported")
        ...
        existing = await self._session.execute(
            select(UserDevice).where(
                ...
                UserDevice.device_public_key == normalized,
        ...
        return await self._device_repo.create(
            customer_id=user.id,
            device_name=device_name.strip()[:120] or "Registered mobile",
            device_public_key=normalized,
            device_status="active",
        )

    async def revoke_device(self, user: User, device_id: int) -> None:
        device = await self._device_repo.get(device_id)
        if device is None or device.customer_id != user.id:
            raise ValueError("Device not found")
        device.device_status = "revoked"
        device.revoked_at = self._now()

    async def _get_challenge(self, raw_challenge: str) -> AuthChallenge:
        if not raw_challenge or len(raw_challenge) > 128:
            raise ValueError("Invalid QR challenge")
        result = await self._session.execute(
            select(AuthChallenge).where(AuthChallenge.challenge_hash == self._hash(raw_challenge))
        )
        challenge = result.scalar_one_or_none()
        if challenge is None:
            raise ValueError("Invalid QR challenge")
        if challenge.challenge_id != raw_challenge:
            raise ValueError("Invalid QR challenge")
    ...
    async def scan(self, raw_challenge: str, user: User, device_id: int) -> AuthChallenge:
        device = await self._device_repo.get(device_id)
        if device is None or device.customer_id != user.id or not device.is_active:
            raise ValueError("Invalid or revoked device")
        challenge = await self._get_challenge(raw_challenge)
        ...
        result = await self._session.execute(
            ...
            .values(status="SCANNED", customer_id=user.id, device_id=device.id, scanned_at=now)
        )
        if result.rowcount != 1:
            raise ValueError("QR challenge was already claimed")
    ...
    async def approve(self, raw_challenge: str, user: User, device_id: int, signature_b64: str) -> AuthChallenge:
        ...
        if device is None or device.customer_id != user.id or not device.is_active:
            raise ValueError("Invalid or revoked device")
        challenge = await self._get_challenge(raw_challenge)
        if challenge.status != "SCANNED" or challenge.customer_id != user.id or challenge.device_id != device.id:
            raise ValueError("QR challenge is not awaiting approval")
        try:
            ...
            public_key.verify(signature, (self.QR_PREFIX + raw_challenge).encode("utf-8"))
        ...
        result = await self._session.execute(
            ...
            .where(
                AuthChallenge.id == challenge.id,
                AuthChallenge.status == "SCANNED",
                AuthChallenge.customer_id == user.id,
        ...
        if result.rowcount != 1:
            raise ValueError("QR challenge could not be approved")
        device.last_seen_at = now
        await self._session.flush()
        return challenge
    ...
    async def decline(self, raw_challenge: str, user: User, device_id: int) -> None:
        ...
        challenge = await self._get_challenge(raw_challenge)
        if challenge.status != "SCANNED" or challenge.customer_id != user.id or challenge.device_id != device.id:
            raise ValueError("QR challenge is not awaiting decision")
        now = self._now()
        result = await self._session.execute(
            update(AuthChallenge)
            .where(AuthChallenge.id == challenge.id, AuthChallenge.status == "SCANNED")
            .values(status="DECLINED", declined_at=now)
        )
        if result.rowcount != 1:
            raise ValueError("QR challenge could not be declined")

    async def cancel(self, raw_challenge: str, browser_session: str) -> None:
        challenge = await self._get_challenge(raw_challenge)
        if challenge.browser_session_id != self._hash(browser_session):
            raise ValueError("Invalid browser challenge session")
        if challenge.status not in {"PENDING", "SCANNED"}:
            return
        challenge.status = "CANCELLED"
        challenge.cancelled_at = self._now()

    async def status_and_consume(self, raw_challenge: str, browser_session: str) -> tuple[str, User | None, tuple[str, str] | None]:
        challenge = await self._get_challenge(raw_challenge)
        if not secrets.compare_digest(challenge.browser_session_id, self._hash(browser_session)):
            raise ValueError("Invalid browser challenge session")
        ...
        if challenge.status != "APPROVED" or challenge.customer_id is None:
            return challenge.status, None, None

        now = self._now()
        result = await self._session.execute(
            update(AuthChallenge)
            .where(AuthChallenge.id == challenge.id, AuthChallenge.status == "APPROVED")
            .values(status="USED", used_at=now)
            .returning(AuthChallenge.customer_id)
        )
        row = result.first()
        if row is None:
