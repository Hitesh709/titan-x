...
class PriceService:
    ...
    async def create_price(
        ...
        existing = await self._session.execute(
            select(DailyPrice).where(
                DailyPrice.symbol == symbol.upper(), DailyPrice.trade_date == trade_date,
            )
        )
    ...
    async def get_prices(
        ...
        skip: int = 0, limit: int = 100, order_by: str = "trade_date", descending: bool = True,
    ...
    async def bulk_import(self, symbol: str, records: list[dict[str, Any]]) -> BulkImportResult:
        ...
        for i, row in enumerate(records):
            ...
            try:
                trade_date: date = row["trade_date"] if isinstance(row["trade_date"], date) else date.fromisoformat(str(row["trade_date"]))
            except (ValueError, KeyError) as exc:
                result.errors.append({"row": i, "errors": [f"invalid trade_date: {exc}"], "data": row})
                continue
            ...
            try:
                ...
                result.errors.append({"row": i, "errors": [str(exc)], "data": row})
    ...
    async def compute_adjusted_prices(self, symbol: str) -> int:
        ...
        action_map: dict[date, float] = {}
        for ca in corp_actions:
            if ca.adjustment_factor:
                action_map[ca.action_date] = ca.adjustment_factor
...
class CorporateActionService:
    ...
    async def create(
        ...
        ratio_numerator: float | None = None, ratio_denominator: float | None = None,
        dividend_amount: float | None = None, adjustment_factor: float | None = None,
    ) -> CorporateAction:
        existing = await self._session.execute(
        ...
        if existing.scalar_one_or_none() is not None:
            raise ValueError(f"Corporate action already exists for {symbol.upper()} on {action_date}")
        return await self._repo.create(
            symbol=symbol.upper(), action_date=action_date, action_type=action_type,
            description=description, ratio_numerator=ratio_numerator,
            ratio_denominator=ratio_denominator, dividend_amount=dividend_amount,
    ...
    async def list_for_symbol(
        ...
        stmt = select(CorporateAction).where(CorporateAction.symbol == symbol.upper()).order_by(CorporateAction.action_date.desc())
        count_stmt = stmt
        total_result = await self._session.execute(count_stmt)
        total = len(total_result.scalars().all())
        stmt = stmt.offset(skip).limit(limit)
