...
def validate_ohlcv(row: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    if row.get("high", 0) < row.get("low", 0): errors.append("high must be >= low")
    if not (row.get("low", 0) <= row.get("open", 0) <= row.get("high", 0)): errors.append("open must be between low and high")
    if not (row.get("low", 0) <= row.get("close", 0) <= row.get("high", 0)): errors.append("close must be between low and high")
    if row.get("volume", 0) < 0: errors.append("volume must be non-negative")
    if any(row.get(k, 0) <= 0 for k in ("open", "high", "low", "close")): errors.append("open, high, low, close must be positive")
    return errors
...
class PriceService:
    ...
    async def create_price(self, symbol: str, trade_date: date, open: float, high: float, low: float, close: float, volume: int) -> DailyPrice:
        ...
        if errors: raise PriceValidationError("; ".join(errors))
        existing = await self._session.execute(select(DailyPrice).where(DailyPrice.symbol == symbol.upper(), DailyPrice.trade_date == trade_date))
        if existing.scalar_one_or_none() is not None: raise ValueError(f"Price already exists for {symbol.upper()} on {trade_date}")
        return await self._price_repo.create(symbol=symbol.upper(), trade_date=trade_date, open=open, high=high, low=low, close=close, volume=volume)

    async def get_prices(self, symbol: str, *, start_date: date | None = None, end_date: date | None = None, skip: int = 0, limit: int = 100, order_by: str = "trade_date", descending: bool = True) -> tuple[Sequence[DailyPrice], int]:
    ...
    async def compute_adjusted_prices(self, symbol: str) -> int:
        ...
        await self._session.flush(); logger.info("adjusted_prices_computed", symbol=symbol, count=count); return count
...
class CorporateActionService:
    ...
    async def create(self, symbol: str, action_date: date, action_type: str, description: str | None = None, ratio_numerator: float | None = None, ratio_denominator: float | None = None, dividend_amount: float | None = None, adjustment_factor: float | None = None) -> CorporateAction:
        ...
        if existing.scalar_one_or_none() is not None: raise ValueError(f"Corporate action already exists for {symbol.upper()} on {action_date}")
