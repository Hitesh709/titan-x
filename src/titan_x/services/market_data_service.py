...
class MarketDataService:
    ...
    async def fetch_and_store_historical(
        ...
        try:
            points = await provider.get_historical_prices(
                symbol, interval="1d", start=start, end=end, synthetic_ok=provider_name == "mock"
            )
        ...
        if company is None:
            self.session.add(
                Company(
                    symbol=symbol,
                    company_name=profile.get("name") or f"{symbol} Corp",
                    isin="IN" + symbol[:10],
    ...
    async def ingest_universe(
            ...
            try:
                results.append(
                    await self.fetch_and_store_historical(
                        symbol, provider_name, api_key, start=start, end=end
                    )
                )
        ...
        return {
            ...
            "inserted_total": sum(result.get("inserted", 0) for result in results),
            "errors": errors,
        }
    ...
    def _normalize_quote_change(q: dict) -> dict:
        ...
        if (
            q.get("change_percent") is None
            and q.get("change") is not None
    ...
    async def get_quotes(self, symbols: list[str]) -> dict:
        ...
        for symbol in symbols:
            ...
            if (
                hit
                and now - hit[0] < _QUOTE_CACHE_TTL_SECONDS
                and hit[1].get("last_price") is not None
            ):
                output.append(hit[1])
            else:
                todo.append(symbol)
        ...
        output.sort(
            key=lambda quote: order.get(
                str(quote.get("symbol", "")).replace(".NS", "").replace(".BO", ""),
                9999,
            )
        )
        return {
    ...
    async def get_company_profile(
        ...
        # Persist provider-discovered profiles so database-backed company
        # endpoints can resolve a symbol immediately after a profile lookup.
        existing = (
            await self.session.execute(select(Company).where(Company.symbol == symbol))
        ).scalar_one_or_none()
        if existing is None:
            self.session.add(
                Company(
                    symbol=symbol,
                    company_name=profile.get("name") or f"{symbol} Corp",
                    isin=profile.get("isin") or "IN" + symbol[:10],
                    exchange=profile.get("exchange") or "NSE",
                    sector=profile.get("sector") or "Unknown",
...
async def load_active_symbols(
    ...
    result = await session.execute(
        select(Company.symbol).where(Company.status == "active").limit(limit)
    )
...
async def run_market_data_ingestion(
    ...
    async with session_factory() as session:
        ...
        result = await MarketDataService(session).ingest_universe(
            symbols,
            provider_name,
