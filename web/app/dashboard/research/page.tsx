...
import TitanRadar from "@/components/dashboard/TitanRadar"
  ...
  const load = useCallback(
    async (silent = false, append = false) => {
      ...
      try {
        ...
        const res = await api.get<ResearchCompanyPage>(`/research/companies?${params.toString()}`)
    ...
    [sortBy, sortDesc, query, items.length],
  )
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {maxDays > 0 && (
          <span className="inline-flex items-center gap-1.5 text-xs text-gray-500">
            <Database size={13} className="text-titan-400" />
            Top company has {maxDays.toLocaleString("en-IN")} data days
          </span>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="glass-card p-10 text-center">
          ...
          <p className="text-sm text-gray-400">
            {query.trim() ? `No company matches "${query}".` : "No companies found. Run a market scan to generate research."}
          </p>
        </div>
      ) : (
        <>
...
function ResearchRow({ company: c }: { company: ResearchCompany }) {
  const dirCls = c.direction === "BUY" ? "badge-green" : c.direction === "SELL" ? "badge-red" : "badge-blue"
  const dirIcon = c.direction === "BUY" ? <TrendingUp size={12} /> : c.direction === "SELL" ? <TrendingDown size={12} /> : <Minus size={12} />
        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
            ...
            <div>
              ...
              <div className="text-sm font-semibold text-white">{c.confidence != null ? `${Math.round(c.confidence)}%` : "—"}</div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="text-sm font-semibold text-white">
                ...
                {c.price_target ? <span className="text-gray-500 font-normal"> · ₹{c.price_target.toLocaleString("en-IN")}</span> : null}
              </div>
            </div>
          </div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            <Link href={`/dashboard/stocks/${c.symbol}`} className="ml-auto text-xs text-titan-400 hover:text-titan-300 inline-flex items-center gap-1">
              View full research →
            </Link>
          </div>
        </>
