...
import { AlertTriangle, ArrowLeft, BookOpen, Building2, CalendarRange, CheckCircle2, Sparkles, TrendingDown, TrendingUp, Activity } from "lucide-react"
  ...
  return (
    <div className="glass-card p-5">
      <div className="flex items-center justify-between gap-3 mb-4">
        <h3 className="text-sm font-semibold text-white flex items-center gap-2"><BookOpen size={16} className="text-titan-400" /> Titan Research</h3>
        {research && <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-titan-600/10 border border-titan-600/25 text-titan-300 text-xs font-medium"><CalendarRange size={12} />{research.days ? `${research.days.toLocaleString("en-IN")} data days` : "research"}</span>}
      </div>
      {!loaded ? <div className="h-16 animate-pulse bg-white/5 rounded-lg" /> : !research ? (
        <p className="text-sm text-gray-500 py-4 text-center">No research yet for {symbol}. Run a market scan to generate an AI recommendation.</p>
      ) : !research.has_research ? (
        <p className="text-sm text-gray-500 py-4 text-center">{research.company_name ?? symbol} does not have a live recommendation yet.</p>
      ) : (
        <>
          <div className="flex flex-wrap items-center gap-2">
            <span className={research.direction === "BUY" ? "badge-green" : research.direction === "SELL" ? "badge-red" : "badge-blue"}>{research.direction ?? "HOLD"}{research.signal ? ` · ${research.signal.replaceAll("_", " ")}` : ""}</span>
