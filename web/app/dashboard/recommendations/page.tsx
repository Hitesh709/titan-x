...
type StrictDeliveryResponse = { recommendations: StockRecommendation[]; strict_technical_threshold?: number; scanning?: boolean; scan_status?: { scanned?: number; universe_size?: number; progress_pct?: number; error?: string | null } }
...
export default function RecommendationsPage() {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (res.scanning) {
        setScanInfo(`Full-market scan running: ${res.scan_status?.scanned ?? 0} / ${res.scan_status?.universe_size ?? 0} symbols (${Math.round(res.scan_status?.progress_pct ?? 0)}%).`)
        ...
        setScanInfo(`Full-market scan complete: ${res.recommendations.length} strict recommendation(s).`)
      }
    } catch (e) {
      if (!mounted.current) return
  ...
  const handleScan = async () => {
    setScanning(true); setError(null); setScanInfo("Starting full-market scan…")
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between"><div><h1 className="text-2xl font-bold text-white">Recommendations</h1><p className="text-gray-500 text-sm mt-1">Titan X strict stock recommendations — Technical Pillar Score must be ≥95 in both delivery and intraday</p></div>{mode === "delivery" && <div className="flex items-center gap-2"><button onClick={handleScan} disabled={scanning} className="btn-secondary text-sm inline-flex items-center gap-2 disabled:opacity-50">{scanning ? <RefreshCw size={14} className="animate-spin" /> : <Play size={14} />}{scanning ? "Scanning…" : "Run Scan"}</button><RefreshButton onClick={handleRefresh} spinning={refreshing} /></div>}</div>
      ...
      {mode === "intraday" ? <IntradayRecommendations /> : <>
        ...
        <SymbolAnalyzer />
        {loading && recommendations.length === 0 ? <div className="glass-card p-8 text-center text-gray-400">Starting full-market scan…</div> : <>
