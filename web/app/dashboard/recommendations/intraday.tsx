...
export function IntradayRecommendations() {
  ...
  const cacheKey = `titanx.strict.intraday.${segment}.v5`

  const load = useCallback(async (retryWhileScanning = true) => {
    setError(null)
    try {
      const endpoint = segment === "fno"
        ? "/recommendations/fno?limit=100"
        : "/recommendations/strict?mode=intraday&segment=equity&limit=100"
      const res = await api.get<StrictResponse>(endpoint)
      ...
      if (retryWhileScanning && segment === "equity" && normalized.scanning) window.setTimeout(() => void load(true), 4000)
    } catch (e) {
      const message = e instanceof Error ? e.message : "Failed to load intraday recommendations"
      setError(message)
  ...
  return <div className="space-y-5">
    ...
    <div className="glass-card p-3 text-xs text-titan-300 border border-titan-500/20">{segment === "fno" ? <>F&O strict gate: <b>Technical Pillar ≥95</b>. Live recommendations are generated from liquid Indian index underlyings (NIFTY, BANKNIFTY, FINNIFTY, MIDCPNIFTY and SENSEX) using 15-minute data. BUY gives a CALL/long-futures bias; SELL gives a PUT/short-futures bias with an ATM strike candidate.</> : <>Intraday strict gate: <b>Intraday Technical Pillar Score ≥95</b>. The score is multi-timeframe: 5m data for 24h/1w/1m plus daily data for 1m/3m, using the technical pillar's RSI, volume, trend/momentum and other indicators. Delivery is independent. Confidence is not used for the 95+ gate. Saved recommendations remain visible after logout/login.</>}</div>
    {segment === "fno" && priorityIndices.length > 0 && <div className="glass-card p-4 border border-titan-500/20"><div className="text-xs uppercase tracking-wider text-titan-300 font-semibold mb-3">Major F&O Indices — Priority</div><div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">{priorityIndices.map((rec) => { const s = score(rec.technical_pillar_score ?? rec.technical_score ?? rec.score); return <div key={`priority-${String(rec.symbol)}`} className="rounded-lg bg-white/[0.03] border border-white/10 px-3 py-2"><div className="text-white font-semibold text-sm">{String(rec.display_name ?? rec.symbol ?? "—")}</div><div className={`text-xs mt-1 ${rec.direction === "BUY" ? "text-emerald-400" : rec.direction === "SELL" ? "text-red-400" : "text-gray-400"}`}>{String(rec.symbol ?? "—")} · {rec.direction ?? "WAIT"} · Technical {s.toFixed(0)}</div></div> })}</div></div>}
    ...
    {!data && !loading && !error && <div className="glass-card p-8 text-center text-sm text-gray-400">No scan has been run yet. Press <b className="text-white">Refresh / Scan</b> to scan.</div>}
    {loading && <div className="glass-card p-8 text-center text-sm text-gray-400">Starting live scan…</div>}
    {!loading && data && !data.scanning && !error && displayedRecommendations.length === 0 && <div className="glass-card p-8 text-center text-sm text-gray-400">No current F&O/equity recommendation meets the Technical Pillar ≥95 strict gate.</div>}
