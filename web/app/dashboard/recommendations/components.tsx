...
interface ParsedMeta {
  ...
  factors?: Record<string, { score?: number; direction?: number; confidence?: number }>
}
...
function parseMeta(rec: StockRecommendation): ParsedMeta {
  ...
  try { return JSON.parse(rec.metadata_json) as ParsedMeta } catch { return {} }
}
...
function parsePillars(rec: StockRecommendation): PillarScore[] {
  ...
  if (rec.inputs_json) {
    try { factors = JSON.parse(rec.inputs_json) as ParsedMeta["factors"] } catch { factors = {} }
  }
  ...
  return PILLAR_ORDER.map((name) => {
    ...
    const score = name === "technical" && technical > 0 ? technical : (typeof raw?.score === "number" ? raw.score : 50)
    return { name, score, direction: typeof raw?.direction === "number" ? raw.direction : 0 }
  })
}

export function StatCard({ label, value, tone, icon }: { label: string; value: number; tone: string; icon: React.ReactNode }) {
  return <div className="glass-card p-4"><div className="flex items-center gap-2 text-gray-500"><span className={tone}>{icon}</span><span className="text-xs">{label}</span></div><div className={`mt-1 text-2xl font-bold ${tone}`}>{value}</div></div>
}
  ...
  return <div className="glass-card p-5 flex flex-col">
    <div className="flex items-center justify-between gap-3">
      <Link href={`/dashboard/stocks/${rec.symbol}`} className="flex items-center gap-3 group">
        ...
        <div><div className="text-white font-semibold group-hover:text-titan-400 transition-colors">{rec.symbol}</div><div className="text-[11px] text-gray-500">₹{rec.current_price?.toLocaleString("en-IN") ?? "—"}</div></div>
      </Link>
