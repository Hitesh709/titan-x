...
function parsePillars(rec: StockRecommendation): PillarScore[] { const strict = rec as StrictRecommendation; const technical = strict.technical_pillar_score ?? strict.technical_score ?? 0; let factors: ParsedMeta["factors"] = {}; if (rec.inputs_json) { try { factors = JSON.parse(rec.inputs_json) as ParsedMeta["factors"] } catch { factors = {} } } if (!Object.keys(factors ?? {}).length) factors = parseMeta(rec).factors ?? {}; return PILLAR_ORDER.map((name) => { const raw = factors?.[name]; const score = name === "technical" && technical > 0 ? technical : (typeof raw?.score === "number" ? raw.score : 50); return { name, score, direction: typeof raw?.direction === "number" ? raw.direction : 0 } }) }
...
function RecommendationCardBase({ rec }: { rec: StockRecommendation }) {
  ...
  const openResearch = () => router.push(researchHref)

  return <div role="button" tabIndex={0} onClick={openResearch} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openResearch() } }} className="glass-card p-5 flex flex-col cursor-pointer hover:border-titan-500/40 transition-colors focus:outline-none focus:ring-1 focus:ring-titan-500/50">
    <div className="flex items-center justify-between gap-3">
      <Link href={researchHref} onClick={(e) => e.stopPropagation()} className="flex items-center gap-3 group min-w-0 rounded-lg -m-1 p-1 hover:bg-white/[0.03]" aria-label={`Open research for ${rec.symbol}`}>
      ...
      <div className="flex items-center gap-2 shrink-0"><Link href={researchHref} onClick={(e) => e.stopPropagation()} className="inline-flex items-center gap-1 text-[11px] text-titan-400 hover:text-titan-300 px-2.5 py-1.5 rounded-md border border-titan-500/25 hover:border-titan-500/50" aria-label={`Research ${rec.symbol}`}>Research <ExternalLink size={11} /></Link><span className={`badge ${dirCls}`}>{rec.direction}{rec.signal ? ` · ${rec.signal.replaceAll("_", " ")}` : ""}</span></div>
    </div>
    <div className="grid grid-cols-3 gap-3 mt-4"><div><div className="text-[11px] text-gray-500 mb-1">Technical Pillar</div><div className="flex items-center gap-2"><div className="flex-1 h-1.5 rounded-full bg-white/5 overflow-hidden"><div className={`h-full rounded-full ${technicalPillar >= DELIVERY_TECHNICAL_THRESHOLD ? "bg-emerald-500" : "bg-titan-500"}`} style={{ width: `${Math.min(100, Math.max(0, technicalPillar))}%` }} /></div><span className="text-xs text-gray-200 font-semibold">{Math.round(technicalPillar)}</span></div></div><div><div className="text-[11px] text-gray-500 mb-1">Confidence</div><div className="text-sm font-semibold text-gray-300">{confidencePct}%</div></div><div><div className="text-[11px] text-gray-500 mb-1">Expected return</div><div className={`text-sm font-semibold ${(rec.predicted_return_pct ?? 0) >= 0 ? "text-emerald-400" : "text-red-400"}`}>{rec.predicted_return_pct !== null && rec.predicted_return_pct !== undefined ? formatPercent(rec.predicted_return_pct) : "—"}</div></div></div>
    ...
    <div className="grid grid-cols-2 gap-3 mt-4 pt-4 border-t border-white/5"><Link href={buyHref} onClick={(e) => e.stopPropagation()} className="btn-primary text-sm text-center">BUY {rec.symbol}</Link><Link href={sellHref} onClick={(e) => e.stopPropagation()} className="btn-secondary text-sm text-center">SELL {rec.symbol}</Link></div>
  </div>
}
