...
def test_canonical_hash_is_deterministic_for_mapping_order() -> None:
    left = {"b": 2, "a": [1, 2]}
    right = {"a": [1, 2], "b": 2}
    assert PredictionAuditService.canonical_hash(left) == PredictionAuditService.canonical_hash(
        right
    )


def test_canonical_hash_changes_when_payload_changes() -> None:
    assert PredictionAuditService.canonical_hash(
        {"value": 1}
    ) != PredictionAuditService.canonical_hash({"value": 2})
