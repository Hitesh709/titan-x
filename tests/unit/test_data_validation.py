...
async def clean_prices(session: AsyncSession):
    ...
    for i in range(730):
        ...
        if d.weekday() < 5 and d <= date.today():
            ...
            session.add(DailyPrice(symbol="CLEAN", trade_date=d, open=p, high=p + 1.5, low=p - 1.5, close=p + 0.3, volume=1_000_000 + idx * 10_000))
            idx += 1
    await session.flush()
    ...
    for i in range(730):
        ...
        if d.weekday() < 5 and d <= date.today():
            ...
            if idx == 10:
                ...
                vol = 100_000_000
            if idx == 20:
                session.add(DailyPrice(symbol="ANOMALY", trade_date=d, open=-1.0, high=p + 1.5, low=p - 1.5, close=p + 0.3, volume=1_000_000))
                idx += 1
                continue
            session.add(DailyPrice(symbol="ANOMALY", trade_date=d, open=p, high=p + 1.5, low=p - 1.5, close=p + 0.3, volume=vol))
            idx += 1
    await session.flush()
...
class TestValidationRun:
    ...
    async def test_validate_clean_dataset(self, svc: DatasetValidationService, clean_prices):
        today = date.today(); start = today - timedelta(days=60)
        run = await svc.validate_dataset("CLEAN", date_from=start, date_to=today)
        assert run.status == "completed"; assert run.anomalies_found == 0; assert run.total_records > 0
        assert run.quality_score is not None; assert run.quality_score >= 75
    ...
    async def test_validate_anomaly_dataset(self, svc: DatasetValidationService, anomaly_prices):
        ...
        assert run.status == "completed"; assert run.anomalies_found > 0
        assert run.missing_values > 0; assert run.price_anomalies > 0; assert run.volume_anomalies > 0

    @pytest.mark.asyncio
    async def test_validate_empty_dataset(self, svc: DatasetValidationService):
        run = await svc.validate_dataset("EMPTY")
        assert run.status == "completed"; assert run.total_records == 0; assert run.quality_score == 0; assert run.quality_rating == "poor"
    ...
    async def test_validate_with_date_range(self, svc: DatasetValidationService, clean_prices):
        ...
        assert run.status == "completed"; assert run.date_from == start; assert run.date_to == end
    ...
    async def test_quality_score_persisted(self, svc: DatasetValidationService, clean_prices):
        ...
        assert len(scores) >= 1; assert scores[0].overall_score >= 75; assert scores[0].run_id == run.id

    @pytest.mark.asyncio
    async def test_list_runs(self, svc: DatasetValidationService, clean_prices):
        today = date.today(); await svc.validate_dataset("CLEAN", date_from=today - timedelta(days=60), date_to=today)
        assert len(await svc.list_validation_runs()) >= 1

    @pytest.mark.asyncio
    async def test_get_run_by_id(self, svc: DatasetValidationService, clean_prices):
        today = date.today(); run = await svc.validate_dataset("CLEAN", date_from=today - timedelta(days=60), date_to=today)
        fetched = await svc.get_validation_run(run.id); assert fetched is not None; assert fetched.id == run.id
...
class TestDuplicateRows:
    ...
    async def test_duplicate_detection_logic(self, svc: DatasetValidationService, session: AsyncSession):
        ...
        await session.flush(); run = await svc.validate_dataset("DUP2"); assert run.duplicate_rows == 0


class TestPriceAnomalies:
    @pytest.mark.asyncio
    async def test_detects_price_spike(self, svc: DatasetValidationService, anomaly_prices):
        assert (await svc.validate_dataset("ANOMALY")).price_anomalies >= 1

    @pytest.mark.asyncio
    async def test_no_price_anomalies_on_clean(self, svc: DatasetValidationService, clean_prices):
        assert (await svc.validate_dataset("CLEAN")).price_anomalies == 0
...
class TestCorpActionMismatch:
    ...
    async def test_detects_mismatch(self, svc: DatasetValidationService, session: AsyncSession):
        ...
        current = (await session.execute(select(DailyPrice).where(DailyPrice.symbol == "CAM", DailyPrice.trade_date == spike_date))).scalar_one()
        previous = (await session.execute(select(DailyPrice).where(DailyPrice.symbol == "CAM", DailyPrice.trade_date < spike_date).order_by(DailyPrice.trade_date.desc()).limit(1))).scalar_one()
        current.open = previous.close
        current.close = previous.close

        session.add(CorporateAction(symbol="CAM", action_date=spike_date, action_type="split", ratio_numerator=2, ratio_denominator=1, description="2:1 split"))
        await session.flush()
    ...
    async def test_no_detection_without_corp_actions(self, svc: DatasetValidationService, clean_prices):
        assert (await svc.validate_dataset("CLEAN")).corp_action_mismatches == 0
...
class TestTimestampMismatch:
    ...
    async def test_detects_missing_days(self, svc: DatasetValidationService, session: AsyncSession):
        ...
        await session.flush(); assert (await svc.validate_dataset("TS")).timestamp_mismatches >= 1
    ...
    async def test_detects_future_date(self, svc: DatasetValidationService, session: AsyncSession):
        ...
        await session.flush(); assert (await svc.validate_dataset("FUTURE")).timestamp_mismatches >= 1

    @pytest.mark.asyncio
    async def test_no_timestamp_issues_on_clean(self, svc: DatasetValidationService, clean_prices):
        assert (await svc.validate_dataset("CLEAN")).timestamp_mismatches == 0


class TestQualityScoring:
    @pytest.mark.asyncio
    async def test_clean_dataset_scores_high(self, svc: DatasetValidationService, clean_prices):
        run = await svc.validate_dataset("CLEAN"); assert run.quality_score >= 75; assert run.quality_rating in ("excellent", "good")

    @pytest.mark.asyncio
    async def test_anomaly_dataset_scores_lower(self, svc: DatasetValidationService, anomaly_prices):
        assert (await svc.validate_dataset("ANOMALY")).quality_score < 99.5

    @pytest.mark.asyncio
    async def test_quality_stats(self, svc: DatasetValidationService, clean_prices):
        await svc.validate_dataset("CLEAN"); stats = await svc.get_quality_stats(); assert "average_score" in stats; assert stats["total_scores"] >= 1

    @pytest.mark.asyncio
    async def test_quality_scores_list(self, svc: DatasetValidationService, clean_prices):
        await svc.validate_dataset("CLEAN"); assert len(await svc.get_quality_scores()) >= 1

    @pytest.mark.asyncio
    async def test_quality_scores_filter_by_symbol(self, svc: DatasetValidationService, clean_prices):
        await svc.validate_dataset("CLEAN"); assert len(await svc.get_quality_scores(symbol="CLEAN")) >= 1


class TestAnomalyQueries:
    @pytest.mark.asyncio
    async def test_get_anomalies_by_run(self, svc: DatasetValidationService, anomaly_prices):
        run = await svc.validate_dataset("ANOMALY"); anomalies = await svc.get_anomalies(run_id=run.id); assert len(anomalies) > 0
        for a in anomalies: assert a.run_id == run.id

    @pytest.mark.asyncio
    async def test_get_anomalies_by_type(self, svc: DatasetValidationService, anomaly_prices):
        run = await svc.validate_dataset("ANOMALY"); anomalies = await svc.get_anomalies(run_id=run.id, anomaly_type="missing_value"); assert len(anomalies) > 0
        for a in anomalies: assert a.anomaly_type == "missing_value"

    @pytest.mark.asyncio
    async def test_get_anomalies_by_severity(self, svc: DatasetValidationService, anomaly_prices):
        run = await svc.validate_dataset("ANOMALY"); assert len(await svc.get_anomalies(run_id=run.id, severity="high")) > 0

    @pytest.mark.asyncio
    async def test_anomaly_stats(self, svc: DatasetValidationService, anomaly_prices):
        await svc.validate_dataset("ANOMALY"); stats = await svc.get_anomaly_stats(); assert "missing_value" in stats or "price_anomaly" in stats or "volume_anomaly" in stats

    @pytest.mark.asyncio
    async def test_get_anomalies_empty(self, svc: DatasetValidationService):
        assert isinstance(await svc.get_anomalies(), list)


class TestClearOldData:
    @pytest.mark.asyncio
    async def test_clear_old_anomalies(self, svc: DatasetValidationService, clean_prices):
        await svc.validate_dataset("CLEAN"); assert await svc.clear_anomalies(older_than_days=0) >= 0
...
class TestEdgeCases:
    ...
    async def test_too_few_records_no_crash(self, svc: DatasetValidationService, session: AsyncSession):
        ...
        if d.weekday() < 5: session.add(DailyPrice(symbol="FEW", trade_date=d, open=100, high=101, low=99, close=100.5, volume=1_000_000))
        await session.flush(); assert (await svc.validate_dataset("FEW")).status == "completed"

    @pytest.mark.asyncio
    async def test_anomaly_has_expected_fields(self, svc: DatasetValidationService, anomaly_prices):
        run = await svc.validate_dataset("ANOMALY"); anomalies = await svc.get_anomalies(run_id=run.id, limit=1)
        if anomalies:
            a = anomalies[0]; assert a.anomaly_type is not None; assert a.severity is not None; assert a.description is not None; assert a.symbol == "ANOMALY"

    @pytest.mark.asyncio
    async def test_multiple_validations_same_symbol(self, svc: DatasetValidationService, clean_prices):
        await svc.validate_dataset("CLEAN"); await svc.validate_dataset("CLEAN"); assert len(await svc.list_validation_runs(symbol="CLEAN")) >= 2
    ...
    async def test_validation_with_zero_price(self, svc: DatasetValidationService, session: AsyncSession):
        d = date.today() - timedelta(days=5)
        if d.weekday() < 5: session.add(DailyPrice(symbol="ZERO", trade_date=d, open=100, high=0.0, low=99, close=100.5, volume=1_000_000))
        await session.flush(); assert (await svc.validate_dataset("ZERO")).missing_values >= 1
